...
//			var itmIdx = $item.index();
            var itmIdx = this.$items.index($item);
            ...
            if (itmIdx !== this.current) {
                ...
                if (this.options.closeIdx !== -1 && itmIdx === this.options.closeIdx) {
                    ...
                    $item.css({'transform':'translate(-200px,500px)'});
                    this.$items.splice(itmIdx, 1);
//					this._rotateSiblings( $item );
                    this._center(parseInt(this.$items.length / 2), true);
                    this.current = -1;
                }
        ...
        _initEvents:function () {
            ...
            var t = function () {
                $(self.$items[i]).css({'left':'0px'});
                i++;
                if (i == 12) {
                    clearInterval(startcard);
                }
